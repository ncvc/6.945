...
(define (numerical-primitive? exp)
  (or (tagged-list? exp '+)
      (tagged-list? exp '-)
      (tagged-list? exp '*)
      (tagged-list? exp '/)))
          ...
          (lambda (subexpression)
            (cond
              ((symbol? subexpression)
                (set! unbound #t)
                subexpression)
              ((self-evaluating? subexpression)
                subexpression)
              (else
                (eval subexpression environment))))
          (operands expression))))

      (if (or unbound (not (for-all? evaluated-subexpressions number?)))
        (cons (operator expression) evaluated-subexpressions)
        (apply (eval (operator expression) environment) evaluated-subexpressions environment)))))
  (lambda (in)
    (and ALLOW-SELF-EVALUATING-SYMBOLS
      (numerical-primitive? in))))
...
;;; 3.1 Apply extension:
;;;  Apply vectors of functions
(defhandler apply
  (lambda (procedure operands calling-environment)
    (vector-map
      (lambda (func)
        (apply func operands calling-environment))
      procedure))
  vector?)

(define default-evaluate-procedure-operand
...
(define evaluate-procedure-operand
  (make-generic-operator 3
			 'evaluate-operand
			  default-evaluate-procedure-operand))
