...
(define (int? var-decl)
  (and (pair? var-decl)
       (memq 'int (cdr var-decl))))

(define (float? var-decl)
  (and (pair? var-decl)
       (memq 'float (cdr var-decl))))

(define (bool? var-decl)
  (and (pair? var-decl)
       (memq 'bool (cdr var-decl))))

(define (str? var-decl)
  (and (pair? var-decl)
       (memq 'string (cdr var-decl))))


(defhandler evaluate-procedure-operand
  (lambda (parameter operand environment)
    (if (integer? operand)
      (default-evaluate-procedure-operand parameter operand environment)
      (error "Operand not an integer: " operand)))
  int?)
...
(defhandler evaluate-procedure-operand
  (lambda (parameter operand environment)
    (if (real? operand)
      (default-evaluate-procedure-operand parameter operand environment)
      (error "Operand not a float: " operand)))
  float?)
...
(defhandler evaluate-procedure-operand
  (lambda (parameter operand environment)
    (if (boolean? operand)
      (default-evaluate-procedure-operand parameter operand environment)
      (error "Operand not a boolean: " operand)))
  bool?)
...
(defhandler evaluate-procedure-operand
  (lambda (parameter operand environment)
    (if (string? operand)
      (default-evaluate-procedure-operand parameter operand environment)
      (error "Operand not a string: " operand)))
  str?)
